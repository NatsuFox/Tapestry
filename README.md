...
```mermaid
flowchart TD
    A[URL输入] --> B[摄取 tapestry-ingest]
    B --> |自动选择爬虫<br/>捕获完整内容+元数据<br/>存储为规范化制品| C[订阅源 tapestry-feed]
    C --> |可选<br/>应用特定来源格式<br/>保留平台上下文| D[综合 tapestry-synthesis]
    D --> |AI驱动的分析<br/>组织到知识库<br/>维护主题层次| E[展示 tapestry-display]
    E --> |生成可浏览的前端<br/>保留结构和导航| F[知识库网站]
...
```mermaid
flowchart TB
    subgraph Layer1["Agent 框架对话层"]
        A[自然语言交互<br/>意图识别<br/>技能调度]
    end

    subgraph Layer2["技能工作流层"]
        B1[INGEST<br/>SKILL.md]
        B2[FEED<br/>SKILL.md]
        B3[SYNTHESIS<br/>SKILL.md]
    end

    subgraph Layer3["共享确定性逻辑层 _src/"]
        C1[registry.py 爬虫注册]
        C2[router.py URL路由]
        C3[fetch.py HTTP请求]
        C4[parse.py HTML解析]
    ...
    subgraph Layer4["平台爬虫实现层 _src/crawlers/"]
        D1[zhihu 知乎]
        D2[x Twitter]
        D3[xiaohongshu 小红书]
        D4[weibo 微博]
    ...
    subgraph Layer5["数据持久化层"]
        E1[captures/ 原始JSON]
        E2[feeds/ 规范化JSON]
        E3[notes/ Markdown]
        E4[knowledge-base/ 知识库]
    end
...
```mermaid
flowchart TD
    A[URL Input] --> B[INGEST tapestry-ingest]
    B --> |Auto-select crawler<br/>Capture full content<br/>Store as artifacts| C[FEED tapestry-feed]
    C --> |Optional<br/>Apply source formatting<br/>Preserve platform context| D[SYNTHESIS tapestry-synthesis]
    D --> |AI-powered analysis<br/>Organize into KB<br/>Maintain topic hierarchy| E[DISPLAY tapestry-display]
    E --> |Generate browsable UI<br/>Preserve structure| F[Knowledge Base Website]
...
```mermaid
flowchart TB
    subgraph Layer1["AI Agent Conversation Layer"]
        A[Natural language interaction<br/>Intent recognition<br/>Skill dispatch]
    end

    subgraph Layer2["Skill Workflow Layer"]
        B1[INGEST<br/>SKILL.md]
        B2[FEED<br/>SKILL.md]
        B3[SYNTHESIS<br/>SKILL.md]
    end

    subgraph Layer3["Shared Deterministic Logic Layer _src/"]
        C1[registry.py Crawler registration]
        C2[router.py URL routing]
        C3[fetch.py HTTP requests]
        C4[parse.py HTML parsing]
        C5[ingest.py Ingest coordinator]
        C6[store.py Filesystem persistence]
        C7[models.py Data models]
        C8[links.py Link extraction]
    end

    subgraph Layer4["Platform Crawler Layer _src/crawlers/"]
        D1[zhihu Zhihu]
        D2[x Twitter]
        D3[xiaohongshu Xiaohongshu]
        D4[weibo Weibo]
        D5[hackernews HN]
        D6[reddit Reddit]
        D7[generic_html Generic]
    end

    subgraph Layer5["Data Persistence Layer"]
        E1[captures/ Raw JSON]
        E2[feeds/ Normalized JSON]
        E3[notes/ Markdown]
        E4[knowledge-base/ KB]
    end
